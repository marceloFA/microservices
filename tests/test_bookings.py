...
class TestBookingService(FlaskTestingCase):
    ...
    def test_booking_record(self):
        ...
        with bookings.app.test_client() as get_booking_route:
            ...
            # blame python json for this ugly shit
            response_json = json.dumps(response.json())
            # tough response is in a list, it contains only one object
            response_booking = bookings.bookings_schema.loads(response_json)[0]
    ...
    def test_new_booking(self):
        ...
        with bookings.app.test_client() as new_booking_route:
            ...
            response_booking = bookings.booking_schema.loads(
                response_book_json)
    ...
    def populate_db(self):
        ...
        b1 = bookings.Booking(
            user=1, date=datetime_date(
                2019, 11, 1), movie=1, rewarded=True)
        b2 = bookings.Booking(
            user=2, date=datetime_date(
                2019, 11, 2), movie=2, rewarded=True)
        b3 = bookings.Booking(
            user=3, date=datetime_date(
                2019, 11, 3), movie=3, rewarded=True)
